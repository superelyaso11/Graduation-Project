...
const dateSimilarity = (data1, data2) => {
...
const calculateMatchScore = (lostItem, foundItem) => {
    ...
    const locationScore = stringsSimilarity(lostItem.location, foundItem.location)
    const dateScore = dateSimilarity(lostItem.date, foundItem.date)

    const score = (locationScore * 0.6) + (dataScore * 0.4) // weight location more than date
...
const matchNewLostItem = async (lostItem) => {
    try{
        ...
        for (const foundItem of foundItems) {
            ...
            if (score >= 0.4) { //only create match if score is good enough
                const existingMatch = await prisma.match.findFirst({    //check if this match already exists
                    where: {
                ...
                if (!existingMatch) {   //avoid duplicate matches
    ...
    try{
        ...
        for (const lostItem of lostItems) {
            ...
            if(score >= 0.4) { //only create match if score is good enough
                ...
                if(!exisitingMatch) { //avoid duplicate matches
                    await prisma.match.create({
                        data: {
                            lostItemId: lostItem.id,
                            foundItemId: foundItem.id,
